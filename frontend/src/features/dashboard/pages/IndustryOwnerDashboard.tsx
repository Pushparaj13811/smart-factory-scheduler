// Industry Owner Dashboard

import { useTranslation } from 'react-i18next';
import { PageHeader } from '@/components/common/PageHeader';
import { StatCard } from '@/components/common/StatCard';
import { ActivityFeed, QuickActions, MachineStatusGrid, ScheduleSummary } from '../components';
import { useDashboardMetrics } from '../hooks/useDashboardMetrics';
import { Wrench, Package, ShoppingCart, TrendingUp } from 'lucide-react';

export function IndustryOwnerDashboard() {
  const { t } = useTranslation('dashboard');
  const { data: metrics } = useDashboardMetrics();

  return (
    <div className="space-y-6">
      <PageHeader
        title={t('welcome.industryOwner')}
        description={t('welcome.description')}
      />

      {/* Metrics Grid */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard
          title={t('stats.totalMachines')}
          value={metrics?.totalMachines?.toString() || '0'}
          description={t('stats.totalMachinesDesc')}
          icon={<Wrench className="h-4 w-4 text-muted-foreground" />}
          trend={
            metrics?.trends?.machinesTrend
              ? {
                  value: metrics.trends.machinesTrend,
                  label: t('stats.fromLastMonth'),
                }
              : undefined
          }
        />

        <StatCard
          title={t('stats.components')}
          value={metrics?.totalComponents?.toString() || '0'}
          description={t('stats.componentsDesc')}
          icon={<Package className="h-4 w-4 text-muted-foreground" />}
        />

        <StatCard
          title={t('stats.activeOrders')}
          value={metrics?.activeOrders?.toString() || '0'}
          description={t('stats.activeOrdersDesc')}
          icon={<ShoppingCart className="h-4 w-4 text-muted-foreground" />}
          trend={
            metrics?.trends?.ordersTrend
              ? {
                  value: metrics.trends.ordersTrend,
                  label: t('stats.fromLastMonth'),
                }
              : undefined
          }
        />

        <StatCard
          title={t('stats.efficiency')}
          value={metrics?.efficiency ? `${metrics.efficiency}%` : '0%'}
          description={t('stats.efficiencyDesc')}
          icon={<TrendingUp className="h-4 w-4 text-muted-foreground" />}
          trend={
            metrics?.trends?.efficiencyTrend
              ? {
                  value: metrics.trends.efficiencyTrend,
                  label: t('stats.fromLastMonth'),
                }
              : undefined
          }
        />
      </div>

      {/* Content Grid */}
      <div className="grid gap-6">
        <QuickActions />

        <div className="grid gap-6 lg:grid-cols-2">
          <MachineStatusGrid />
          <ScheduleSummary />
        </div>

        <ActivityFeed />
      </div>
    </div>
  );
}
